...
mut_thresh <- floor(2 * length(pc_celllines) / 100)
...
# IMPORTANT: Computationally expensive!!!!
...
pc_data$mutation_annot <- left_join(mut_mat_annot,all_cancers_mut_df,
                          ...
                          dplyr::group_by(Hugo_Symbol,Cell_Line) %>% 
                          dplyr::summarise(Variant_Classification = paste(Variant_Classification, collapse=";"), n = n()) 
...
```{r sl_pancancer}
hits_pancan <- slidR::identifySLHits(canc_data = pc_data, n_cand = 30, path_results = path_results)
